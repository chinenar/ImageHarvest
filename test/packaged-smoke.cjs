...
(async () => {
  ...
  try {
    ...
    for (const id of ['removeSelected','clearResults','undoRemove','hideChrome','mainOnly','sourceFilter','browserMode']) assert.equal(await page.locator('#'+id).count(),1);
    assert.equal(settings.data.version, '1.0.5');
